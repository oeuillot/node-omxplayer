...
OMXPlayer.prototype.start = function(moviePath, options, callback) {
	...
	if (omxProcess) {
		...
		if (oldPlaybackStatus !== "Stop") {
			this.properties.PlaybackStatus = "Stop";
			this.emit("prop:PlaybackStatus", "Stop", oldPlaybackStatus);
			this.emit("property", {
				PlaybackStatus: "Stop"
			}, {
	...
	if (configuration.omxPlayerPath) {
		...
		this._log("Execute  '" + configuration.omxPlayerPath + "' parameters=", parameters, " spawnOptions=",
	...
	if (!configuration.dbus) {
		...
		this._log("Dbus address=", configuration.dbus.busAddress);
	}
	...
	setTimeout(function waitSync() {
		...
		self.getIdentity(function(error, id) {
			if (error || !id) {
				self._error("Waiting omxplayer launch error=", error);
...
OMXPlayer.prototype._poolProperties = function(ps, callback) {
	...
	function property() {
		if (!ps.length) {
			if (modifiedCount) {
				self._log("Emit 'property' newValues=", newValues, " oldValues=", oldValues);
		...
		self["get" + name].call(self, function(error, value) {
			...
			if (properties[name] !== value) {
				...
				self._log("Emit 'prop:" + name + "' newValue=", value, " oldValue=", oldValues[name]);
...
var omxCommandsParameter0 = [ "Next",
	...
	"HideSubtitles",
	"Quit" ];
var omxCommandsParameter1 = [ 'Position', 'Seek', 'VideoPos', 'SelectSubtitle', 'SelectAudio', 'Action', 'Start' ]
...
omx.prototype._express = function(req, res, next) {
	...
	if (!expressPath || req.path.indexOf(expressPath)) {
		return next();
	}

	// replace + and decode
	var path = req.path.substring(expressPath);
	...
	console.log("Perform command cmdName=", cmdName, " path=", path, " reqPath=", reqPath);
	...
	if (this.properties[cmdName] !== undefined) {
		res.writeHead(200, {
			'Content-Type': 'application/json',
			'Cache-Control': NO_CACHE_CONTROL
		});

		if (path && this.properties["set" + command]) {
			this.properties["set" + command](path, resEnd);
			return;
		}
	...
	if (dbusCommands.indexOf(command) >= 0 || omxCommandsParameter0.indexOf(command) >= 0 ||
			omxCommandsParameter1.indexOf(command) >= 0) {
		var c2 = command.substring(0, 1).toLowerCase() + command.substring(1);
		...
		if (omxCommandsParameter0.indexOf(command) >= 0) {
			return this[c2](resEnd);
		}

		if (command === "play" && this._configuration.moviesBasePath) {

			path = this._configuration.moviesBasePath + path.replace("/", Path.sep);
		}

		return this[c2](path, resEnd);
	}
