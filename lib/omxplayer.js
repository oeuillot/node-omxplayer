	...
	setTimeout(function waitSync() {
		...
		self.getIdentity(function(error, id) {
			...
			self._notificationsIntervalId = setInterval(function() {
				self._poolProperties();
			}, 1000);
			var ps = [].concat(omxProperties, omxProperties2);
			self._poolProperties(ps);
...
OMXPlayer.prototype._poolProperties = function(ps) {
	var self = this;
	var properties = self.properties;

	ps = ps || [ "Volume", "Position", "Duration", "PlaybackStatus" ];
...
var omxProperties2 = [ "ListSubtitles", "ListAudio", "ListVideo" ];
omxProperties2.forEach(function(name) {
	OMXPlayer.prototype["get" + name] = function(callback) {
		assert(typeof (callback) === "function", "Invalid callback parameter");
