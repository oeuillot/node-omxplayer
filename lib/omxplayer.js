...
OMXPlayer.prototype.start = function(moviePath, callback) {
	...
	setTimeout(function waitSync() {
		...
		self.getIdentity(function(error, id) {
			...
			self._oldProperties = {};
			self._notificationsIntervalId = setInterval(self._poolProperties.bind(self), 1000);
...
OMXPlayer.prototype._poolProperties = function() {
	var self = this;
	var oldProperties = self._oldProperties;
	[ "Volume", "Position", "Duration", "PlaybackStatus" ].forEach(function(name) {
		console.log("Get ", name, self["get" + name]);
