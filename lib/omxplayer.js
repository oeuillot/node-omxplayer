...
OMXPlayer.prototype.start = function(moviePath, options, callback) {
	...
	if (configuration.omxPlayerPath) {
		...
		if (this.properties.Volume !== undefined) {
			parameters.push('--vol', this.properties.Volume);
...
OMXPlayer.prototype._express = function(req, res, next) {
	...
	if (this.properties && this.properties[cmdName] !== undefined) {
		...
		if (path && this.properties["set" + cmdName]) {
			this.properties["set" + cmdName](path, resEnd);
			return;
		}
	...
	if (dbusCommands.indexOf(cmdName) >= 0 || omxCommandsParameter0.indexOf(cmdName) >= 0 ||
			omxCommandsParameter1.indexOf(cmdName) >= 0) {
		...
		if (omxCommandsParameter0.indexOf(cmdName) >= 0) {
			return this[c2](resEnd);
		}

		if (cmdName === "play" && this._configuration.moviesBasePath) {
